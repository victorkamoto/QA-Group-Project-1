...
export async function createTask(task: any): Promise<ApiRes> {
  try {
...
export async function getTasksByProjectId(id: string): Promise<ApiRes> {
  try {
    const response = await client.get(`/tasks?projectId=${id}`);
    return {
      status: response.status,
      message: response.data?.message,
      data: response.data?.details,
