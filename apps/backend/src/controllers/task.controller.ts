...
export const patchTask = async (req: Request, resp: Response) => {
  const id: string = req.params.id;
  const body: UpdateTask = req.body;

  try {
    const { code, message, details } = await patch(id, body);
...
export const deleteTask = async (req: Request, resp: Response) => {
  try {
    const { code, message, details } = await deleteTaskFromProject(
      req.params.id
    );
...
export const updateStatus = async (req: Request, resp: Response) => {
  try {
    const { code, message, details } = await updateTaskStatus(
      req.params.id,
      req.body.status
    );
